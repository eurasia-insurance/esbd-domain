...
CREATE TABLE POLICY_VEHICLE (ID INTEGER NOT NULL, CURRENTODOMETERVALUE INTEGER, VEHICLEAGECLASS VARCHAR(255), VEHICLECLASS VARCHAR(255), VEHICLEPURPOSE VARCHAR(255), VEGICLE_CERTIFICATE_DATE_OF_ISSUE DATE, VEGICLE_CERTIFICATE_NUMBER VARCHAR(255), VEGICLE_CERTIFICATE_MAJOR_CITY TINYINT(1) default 0, VEGICLE_CERTIFICATE_REGNUM VARCHAR(255), VEGICLE_CERTIFICATE_REGION VARCHAR(255), CREATED_INSTANT DATETIME, CREATED_AUTHOR_ID INTEGER, MODIFIED_INSTANT DATETIME, MODIFIED_AUTHOR_ID INTEGER, INSURER_ID INTEGER, VEHICLE_ID INTEGER, POLICY_ID INTEGER, PRIMARY KEY (ID))
CREATE TABLE SUBJECT (ID INTEGER NOT NULL, DTYPE VARCHAR(31), COMMENTS VARCHAR(255), ECONOMICSSECTOR VARCHAR(255), IDNUMBER VARCHAR(255), RESIDENT TINYINT(1) default 0, TAXPAYERNUMBER VARCHAR(255), CONTACT_EMAIL VARCHAR(255), CONTACT_HOME_ADDRESS VARCHAR(255), CONTACT_PHONE VARCHAR(255), CONTACT_SITE_URL VARCHAR(255), ORIGIN_CITY VARCHAR(255), ORIGIN_COUNTRY VARCHAR(255), PRIMARY KEY (ID))
CREATE TABLE SUBJECT_COMPANY (ID INTEGER NOT NULL, ACCOUNTANTNAME VARCHAR(255), COMPANYNAME VARCHAR(255), HEADNAME VARCHAR(255), COMPANYACTIVITYKIND_ID INTEGER, PRIMARY KEY (ID))
...
CREATE TABLE USER (ID INTEGER NOT NULL, AUTHENTIFICATED TINYINT(1) default 0, LASTACTIVITY DATETIME, LASTSESIONID VARCHAR(255), LOGIN VARCHAR(255), BRANCH_ID INTEGER, ORGANIZATION_ID INTEGER, SUBJECT_ID INTEGER, PRIMARY KEY (ID))
CREATE TABLE VEHICLE (ID INTEGER NOT NULL, COLOR VARCHAR(255), REALEASEDATE DATE, STEERINGWHEELLOCATION VARCHAR(255), VEHICLECLASS VARCHAR(255), VINCODE VARCHAR(255), VEHICLE_ENGINE_NUMBER VARCHAR(255), VEHICLE_ENGINE_POWER INTEGER, VEHICLE_ENGINE_VOLUME INTEGER, VEHICLEMODEL_ID INTEGER, PRIMARY KEY (ID))
CREATE TABLE VEHICLE_MANUFACTURER (ID INTEGER NOT NULL, FOREIGN TINYINT(1) default 0, NAME VARCHAR(255), PRIMARY KEY (ID))
CREATE TABLE VEHICLE_MODEL (ID INTEGER NOT NULL, NAME VARCHAR(255), MANUFACTURER_ID INTEGER, PRIMARY KEY (ID))
...
ALTER TABLE SUBJECT_COMPANY ADD CONSTRAINT FK_SUBJECT_COMPANY_COMPANYACTIVITYKIND_ID FOREIGN KEY (COMPANYACTIVITYKIND_ID) REFERENCES DICTIONARY (ID)
...
ALTER TABLE VEHICLE ADD CONSTRAINT FK_VEHICLE_VEHICLEMODEL_ID FOREIGN KEY (VEHICLEMODEL_ID) REFERENCES VEHICLE_MODEL (ID)
ALTER TABLE VEHICLE_MODEL ADD CONSTRAINT FK_VEHICLE_MODEL_MANUFACTURER_ID FOREIGN KEY (MANUFACTURER_ID) REFERENCES VEHICLE_MANUFACTURER (ID)
